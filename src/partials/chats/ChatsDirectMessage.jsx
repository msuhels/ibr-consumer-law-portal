      ...
      <ul className="mb-6">
        {allUsers?.map((val, i) => {
          ...
          return (
            <div key={i}>
              <li className="-mx-2">
                <button className={`flex items-center justify-between w-full p-2  ${chatID == val._id ? 'bg-[#EEF0F0] rounded-3xl border border-[#D4D5DB]' : ""} `} onClick={() => { starChating(val?._id) }} >
                  <div className={`flex items-center truncate ${chatID == val._id ? 'pl-6':''}`}>
                    <img className="w-10 h-10 rounded-full mr-2" src={img} width="40" height="40" alt="User 01" />
                    <div className='text-left'>
                      <p className='font-bold text-[14px] text-[#080D18]'>{name} {secondname ? `| ${secondname}` : ''}</p>
                    </div>
                  </div>
...
export default ChatsDirectMessages;