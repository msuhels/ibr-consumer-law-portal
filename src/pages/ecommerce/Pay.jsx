...
function Pay({ BackendUrl }) {
  ...
  const handleSubmit = async (e) => {
    ...
    let amountToCut = "";
    amountToCut = isPlanAmount;

    // if (user.role === "mentee") {
    //   amountToCut = isPlanAmount
    // }
    // else if (state?.plan_type === "YEARLY") {
    //   amountToCut = isPlanAmount
    // } else {
    //   amountToCut = 67
    // }
    ...
    try {
      const res = await axios.post(
        `${BackendUrl}/user/authorize-payment`,
        {
          form,
          email: user?.email,
          type: state.plan_type,
          amount: amountToCut,
          subscription_amount: state.subscription_amount,
          trial: state.istrial,
          planName: state.plan_id,
        },
        {
      ...
      console.log(state.plan_id, "state.plan_id")
      console.log(res?.data, "state.plan_id")
    ...
    <>
      ...
      <main className="grow bg-white">
        <div className="relative pt-10	pb-10	">
          <div
            className="absolute inset-0 bg-slate-800 overflow-hidden bg-white"
            aria-hidden="true"
          >
                  ...
                  <div className="col-lg-12">
                    <div className="">
                      ...
                      <p className="mt-3 mb-4">
                        ...
                        You are few steps away from unlocking your access to the
                        #1 Consumer Law & AI-powered Dispute Software!
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white dark:bg-slate-800 px-8 pb-6 pt-6 rounded-3xl shadow-lg border ">
            {card && (
              <div>
                ...
                {(state?.plan_name === "1" && state?.price === 1) ||
                  (state?.plan_name === "5" && state?.price === 1) ? (
                  ""
                ) : (
                  <>
                    {isCouponCodeValid ? (
                ...
                <div className="mt-6">
                  <div className="mb-4">
                    {loaderSignin ? (
                      <button
                        className="flex justify-center py-2 rounded-3xl w-full bg-indigo-500 hover:bg-indigo-600 text-white disabled:border-slate-200 dark:disabled:border-slate-700 disabled:bg-slate-100 dark:disabled:bg-slate-800 disabled:text-slate-400 dark:disabled:text-slate-600 disabled:cursor-not-allowed shadow-none"
                        disabled
                      >
                        <span className=" flex ml-2 text-center">
                          <svg
                            className="animate-spin w-4 h-4 fill-current shrink-0 me-2 mt-1 "
                            viewBox="0 0 16 16"
                          >
                            ...
                            {isCouponCodeValid ? (
                              ...
                              <>
                                {user.role === "mentee" ? (
                                  ...
                                  <>
                                    {/* Pay $ {state?.plan_type === "YEARLY" ? isPlanAmount : "67"} */}
                                    Pay $ {isPlanAmount}
                                  </>
                                )}
                              </>
                ...
                {(state?.plan_name === "1" && state?.price === 1) ||
                  ...
                  <div className="space-x-4 text-center">
                    No charges will be applied today. Automatic renewal will
                    occur in 14 days. You can cancel anytime.
                  </div>
                ) : (
                  <>
                    {isCouponCodeValid && (
                      <div className="space-x-4 text-center">
                        Note: Promotion is applicable on first payment only.
                        From next cycle regular amount will be charged.
                      </div>
                    )}
                  </>
                )}
                <div className="flex justify-center items-center py-4">
                  <div>
                    <a
                      ...
                      href="consumerlawdispute.ai/disclaimer"
                    >
                      Disclaimer
                    </a>
                  </div>
                  ...
                  <div>
                    <a
                      className="text-blue-500"
                      href="https://www.consumerlawdispute.ai/terms-conditions/"
                    >
                      Terms and condition
                    </a>
                  </div>
